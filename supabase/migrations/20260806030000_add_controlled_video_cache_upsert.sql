...
begin
  ...
  if pg_column_size(p_payload) > 6291456 then
    raise exception 'VIDEO_CACHE_PAYLOAD_TOO_LARGE';
  end if;
  if p_payload ? 'title' and length(coalesce(p_payload ->> 'title', '')) > 500 then
    raise exception 'VIDEO_CACHE_TITLE_TOO_LONG';
  end if;
  if p_payload ? 'summary' and length(coalesce(p_payload ->> 'summary', '')) < 100 then
    raise exception 'VIDEO_CACHE_SUMMARY_TOO_SHORT';
  end if;
  if p_payload ? 'summary' and length(coalesce(p_payload ->> 'summary', '')) > 200000 then
    raise exception 'VIDEO_CACHE_SUMMARY_TOO_LONG';
  end if;
  if p_payload ? 'raw_subtitle' and jsonb_typeof(p_payload -> 'raw_subtitle') <> 'array' then
    raise exception 'INVALID_RAW_SUBTITLE';
  end if;
  if p_payload ? 'processed_subtitle' and jsonb_typeof(p_payload -> 'processed_subtitle') <> 'array' then
    raise exception 'INVALID_PROCESSED_SUBTITLE';
  end if;
  if p_payload ? 'segments' and jsonb_typeof(p_payload -> 'segments') <> 'array' then
    raise exception 'INVALID_SEGMENTS';
  end if;
  if p_payload ? 'raw_subtitle' and jsonb_array_length(p_payload -> 'raw_subtitle') > 20000 then
    raise exception 'RAW_SUBTITLE_TOO_LARGE';
  end if;
  if p_payload ? 'processed_subtitle' and jsonb_array_length(p_payload -> 'processed_subtitle') > 20000 then
    raise exception 'PROCESSED_SUBTITLE_TOO_LARGE';
  end if;
  if p_payload ? 'segments' and jsonb_array_length(p_payload -> 'segments') > 1000 then
    raise exception 'SEGMENTS_TOO_LARGE';
  end if;
  ...
  do update set
    ...
    subtitle_upload_count = coalesce(target.subtitle_upload_count, 0)
      + case when coalesce((p_payload ->> 'increment_subtitle_upload_count')::boolean, false) then 1 else 0 end,
    ...
    summary_call_count = coalesce(target.summary_call_count, 0)
      + case when coalesce((p_payload ->> 'increment_summary_call_count')::boolean, false) then 1 else 0 end,
    ...
    segments_call_count = coalesce(target.segments_call_count, 0)
      + case when coalesce((p_payload ->> 'increment_segments_call_count')::boolean, false) then 1 else 0 end,
    ...
    rumors_call_count = coalesce(target.rumors_call_count, 0)
      + case when coalesce((p_payload ->> 'increment_rumors_call_count')::boolean, false) then 1 else 0 end,
    updated_at = now();
end;
$$;
